...
class photoaparatSpider(scrapy.Spider):
    name = 'photoaparat'
    allowed_domains = ['ek.ua']
    BASE_URL = 'https://ek.ua/ua'
    start_urls = ['https://ek.ua/ua/list/206/']
    ...
    def parse(self, response):
        ...
        for i in range(0, 2):
            yield SeleniumRequest(
                ...
                callback=self.parse_photoaparat,
                wait_time=10,
                wait_until=expected_conditions.element_to_be_clickable(
                    (By.CSS_SELECTOR,
                     ".model-shop-name .sn-div")
    ...
    def parse_photoaparat(self, response):
        soup = BeautifulSoup(response.text, 'html.parser')
        photoaparat_list = soup.find(id="list_form1").find_all('div')
        for photoaparat in photoaparat_list:
            try:
                img_url = photoaparat.find(class_="list-img").find('img').get('src')
                model_wrapper = photoaparat.find(class_="model-short-info").find(class_="model-short-title no-u")
                model = model_wrapper.find('span').getText()
                model_url = model_wrapper.get('href')
                price = photoaparat.find(class_="model-hot-prices-td").find(class_="model-price-range").find('a').find_all(
                    'span')
                start_price = int(price[0].getText().replace('\xa0', ''))
                end_price = int(price[1].getText().replace('\xa0', ''))
                shops = []
                shops_html = photoaparat.find(class_="model-hot-prices").find_all('tr')
